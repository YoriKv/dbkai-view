#!/usr/bin/env python3
"""Freeze DBKai into a standalone app with PyInstaller.
...
release ships — there is no second copy of the flags to drift out of sync.
    ...
    python3 packaging/build.py             # -> dist/dbkai (or dist/dbkai.app)
...
PACKAGE = REPO_ROOT / "app" / "dbkai"
VERSION_FILE = PACKAGE / "__init__.py"
ENTRY_POINT = PACKAGE / "__main__.py"
# The same image the app uses as its window icon at runtime. PyInstaller turns
# it into the platform's format (.ico / .icns) through Pillow, so no converted
# copies are kept in the repository.
ICON = PACKAGE / "resources" / "icons" / "app.png"
APP_NAME = "dbkai"
...
def icon_option() -> list:
    ...
    if sys.platform in ("win32", "darwin"):
        # Embedded into the .exe (Explorer, taskbar) or baked into dbkai.app's
        # Info.plist (Dock, Finder).
...
def pyinstaller_command() -> list:
    ...
    imports: the package's data files (the icons under ``dbkai/resources``)
    would otherwise be left out and the frozen app would start without them.
    PySide6's and PyOpenGL's bundled hooks pull in the Qt plugins and the GL
    platform modules.

    pyinstaller and pillow (for the icon conversion) come in ephemerally via
    ``--with``, so neither need live in the dev dependency group or uv.lock.
    """
    return [
        "uv",
        "run",
...
def make_archive(*, dry_run: bool) -> Path:
    ...
    if sys.platform == "darwin":
        ...
        # bundle. --keepParent keeps dbkai.app as the archive's top level.
        run(
...
def main(argv: list | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="build.py",
        description=(
            "Freeze DBKai into a standalone app with PyInstaller, the same way "
            "the release workflow does. Builds for the OS it runs on."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "examples:\n"
            "  python3 packaging/build.py            build dist/dbkai\n"
    ...
    parser.add_argument(
        ...
        "--clean",
        action="store_true",
        help="delete build/, dist/ and dbkai.spec first, for a from-scratch build",
    )
    parser.add_argument(
        "-n",
        "--dry-run",
